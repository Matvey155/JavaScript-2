...
let ar1 = ["строка", 10, true, "еще строка", false, 5];
...
let ar2 = ["строка", 456, true, "еще строка", false];
...
function f9() {
    let lastElementIndex = ar9.length - 1;
    document.querySelector('.out-9').textContent = ar9[lastElementIndex];
}
...
function f10() {
    let lastElementIndex = ar10.length - 1;
    let sum = ar10[1] + ar10[lastElementIndex];
...
function f18() {
    let maxNumber = ar18[0];
    for (let i = 1; i < ar18.length; i++) {
        if (ar18[i] > maxNumber) {
            maxNumber = ar18[i];
        }
    }
    document.querySelector('.out-18').innerHTML = maxNumber;
}
