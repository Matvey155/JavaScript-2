...
function f7() {
    const poppedElement = d7[d7.length - 1];
...
function f13() {
    let reversedArr = d13.slice().reverse();
    showArr('.out-13', d13);
}
...
function f18() {
    ...
    let result = d18.includes(inputValue);
}
...
function f19() {
    // maxString = d19.reduce((a, b) => a.length > b.length ? a : b);
    maxString = d19[0];
